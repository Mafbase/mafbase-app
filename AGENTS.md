...
4. **Генерация кода**: После добавления новых строк в `.arb` файл запустите `flutter gen-l10n`

# Правила проверки кода

## Проверка ошибок

1. **Всегда используйте `flutter analyze` для проверки ошибок** - это основной инструмент для статического анализа кода
2. **После внесения изменений в код запускайте `flutter analyze`** для проверки на наличие ошибок компиляции и предупреждений
...
3. **Обновляйте локализацию** - после изменений в `.arb` всегда запускайте `flutter gen-l10n`

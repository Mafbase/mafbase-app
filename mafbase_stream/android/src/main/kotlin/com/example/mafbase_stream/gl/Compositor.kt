...
internal class Compositor(
    ...
    // SurfaceTexture.timestamp от Camera2 идёт в шкале BOOTTIME, если sensor сообщает
    // SENSOR_INFO_TIMESTAMP_SOURCE = REALTIME, и в шкале CLOCK_MONOTONIC при UNKNOWN.
    // AAC PTS считается от SystemClock.elapsedRealtimeNanos (BOOTTIME). Если устройство
    // успело уйти в deep sleep с момента загрузки, разница BOOTTIME-MONOTONIC > 0, и на
    // UNKNOWN-устройствах это даёт постоянное смещение видео относительно аудио (звук
    // «опережает» картинку на накопленный sleep). Чинится приведением видео-PTS к
    // BOOTTIME — определяем шкалу по первому кадру и дальше всегда добавляем offset.
    private var tsBootMinusMonoNs: Long = 0L
    private var tsScaleResolved: Boolean = false
    ...
    private fun drawFrame() {
        ...
        for ((id, output) in outputs) {
            ...
            if (output.needsPresentationTime) {
                EGLExt.eglPresentationTimeANDROID(eglDisplay, output.eglSurface, normalizeTimestampNs(st.timestamp))
            }
    ...
    private fun normalizeTimestampNs(rawNs: Long): Long {
        if (!tsScaleResolved) {
            val nowBoot = SystemClock.elapsedRealtimeNanos()
            val nowMono = System.nanoTime()
            val distBoot = abs(nowBoot - rawNs)
            val distMono = abs(nowMono - rawNs)
            tsBootMinusMonoNs = if (distMono < distBoot) (nowBoot - nowMono) else 0L
            tsScaleResolved = true
            val scale = if (tsBootMinusMonoNs == 0L) "REALTIME" else "MONOTONIC"
            Log.i(TAG, "camera timestamp scale: $scale, offset=${tsBootMinusMonoNs / 1_000_000}ms")
        }
        return rawNs + tsBootMinusMonoNs
    }

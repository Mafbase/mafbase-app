...
class WaitForPaymentStatus extends $pb.ProtobufEnum {
  static const WaitForPaymentStatus pending = WaitForPaymentStatus._(0, _omitEnumNames ? '' : 'pending');
  static const WaitForPaymentStatus succeeded = WaitForPaymentStatus._(1, _omitEnumNames ? '' : 'succeeded');
  static const WaitForPaymentStatus canceled = WaitForPaymentStatus._(2, _omitEnumNames ? '' : 'canceled');

  static const $core.List<WaitForPaymentStatus> values = <WaitForPaymentStatus>[
    pending,
    succeeded,
    canceled,
  ];

  static final $core.List<WaitForPaymentStatus?> _byValue = $pb.ProtobufEnum.$_initByValueList(values, 2);
  static WaitForPaymentStatus? valueOf($core.int value) =>
      value < 0 || value >= _byValue.length ? null : _byValue[value];

  const WaitForPaymentStatus._(super.value, super.name);
}

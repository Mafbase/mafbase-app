...
class ClubGamesBloc extends Bloc<ClubGamesEvent, ClubGamesState> {
  ...
  ClubGamesBloc(super.initialState, this.repository) {
    on<ClubGamesEventInit>(_init);
  }

  Future<void> _init(ClubGamesEventInit event, Emitter<ClubGamesState> emit) async {
    emit(state.copyWith(loading: true, sort: event.sort));
    try {
      final games = await repository.getGames(clubId: event.clubId, range: event.range, sort: event.sort);
